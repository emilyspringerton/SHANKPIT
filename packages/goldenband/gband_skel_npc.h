...
// gband_skel_npc_load_kit loads <asset_dir>/<mesh_name>.gskel + .gmesh, plus two real animation
// clips (+ their own .gband.json manifests, for real channel names): <idle_clip_name> looped
// while stationary, <walk_clip_name> looped while moving. Returns the new kit's real index
// (0-based, stable for the process lifetime) on success, or -1 on failure (missing/malformed
// asset, a skeleton with more joints than GSKEL_MAX_JOINTS, or GBAND_SKEL_NPC_MAX_KITS already
// reached).
...
// look, so a stale animation player is never sampled against the wrong clip/skeleton). Movement
// is detected the same real way gband_mesh_rig_draw already does: a per-slot previous-position
// comparison against a small epsilon, not an explicit "is this NPC walking" flag from the caller.
