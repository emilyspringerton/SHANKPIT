...
## 2026-09-15
- S459-67: fixed residual jump "rubberbanding" after S459-65's own hard-snap threshold fix -- founder real-time (after testing S459-65): "yea i etsted it its possible its a little better not sure not fixed". Real structural gap found: NetPlayer (the wire packet) never carries velocity or grounded state, only position -- client reconciliation resets p->x/y/z from the server but has never touched p->vy, so the jump-arc replay kept simulating from whatever locally-predicted velocity the client already had, silently diverging from the server's real velocity (external impulses the client can't predict, plus ordinary drift) with nothing to correct it. Fixed by re-seeding p->vx/vy/vz from a real finite-difference estimate between two confirmed authoritative positions (using the client's own recorded cmd timestamps) before replay runs, instead of trusting potentially-diverged local prediction -- reset on connect/respawn (a real position discontinuity, not real motion to diff against). apps/lobby builds clean, no new warnings. Real, honest limit: needs a human playtest to confirm the actual in-game feel, same as S459-65 (sess-20260905-0720-ec33e7c5)
